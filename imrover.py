...
def main_traa2nuc(args):
    ...
    if args.cpos_list:
        if args.skipheader:
            args.cpos_list.readline()

        for line in args.cpos_list:

            pair = line.strip().split(args.delim)
            if args.colgene>0 and args.colcpos>0:
...
def add_std_arguments(psr):

    psr.add_argument('-a', metavar='annotation',
                     required = True,
...
if __name__ == '__main__':
    ...
    psrs = parser.add_subparsers()

    psr_codoneq = psrs.add_parser("codon.eq", help="test whether two codon annotations may be the same nucleotide position", epilog="Example: prog -c MET.p1010 MET.p992 -a hg19.map")
    psr_codoneq.add_argument("-c", nargs=2, help="two codons to test. Format: [gene_name].p[codon_location], e.g., MET.p1010", dest='codons')
    add_std_arguments(psr_codoneq)
    psr_codoneq.set_defaults(func=main_codoneq)

    psr_srchalt = psrs.add_parser('search.alt.codon', help='search alternative codons')
    psr_srchalt.add_argument('-c', dest='codon_list', help='a list of codons to search alternatives')
    add_std_arguments(psr_srchalt)
    psr_srchalt.set_defaults(func=main_srchalt)



    psr_traa2nuc = psrs.add_parser("tr.aa2nuc", help='translate from amino acid (codon) position(s) to nucleotide position(s)', epilog="""
python isoform_resolver.py tr.aa2nuc -c KRAS:12 -a hg19.map
""")
    psr_traa2nuc.add_argument('-t', dest="cpos_list", default=None, type = FileType('r'), help='codon position table')
    ...
    psr_trnuc2aa = psrs.add_parser('tr.nuc2aa', help='translate from nucleotide position(s) to amino acid (codon) position(s)', epilog="""
python isoform_resolver.py tr.nuc2aa -n chr12:25398285 -a hg19.map
""")
    psr_trnuc2aa.add_argument('-t', dest="npos_list", type = FileType('r'), default=None, help='nucleotide position table')
    ...
    psr_trnuc2aa.set_defaults(func=main_trnuc2aa)
